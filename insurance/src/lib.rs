...
impl Insurance {
    ...
    /// Deactivate a policy and its associated premium schedule.
    ///
    /// This function is idempotent: if the policy is already inactive, it will
    /// return `false` without emitting duplicate events or updating storage.
    /// If the policy has an associated active schedule, it will be deactivated.
    pub fn deactivate_policy(env: Env, caller: Address, policy_id: u32) -> bool {
        ...
        // Idempotency check: if already inactive, return false and stop
        if !policy.active {
            return false;
        }
        ...
        // Automatically deactivate associated schedule if it exists
