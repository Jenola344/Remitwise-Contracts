...
/// Premium payment interval in seconds (30 days).
const PREMIUM_INTERVAL_SECONDS: u64 = 30 * 24 * 60 * 60;
...
impl InsuranceContract {
    ...
    pub fn create_policy(
        ...
        let next_payment_due = now.saturating_add(PREMIUM_INTERVAL_SECONDS);
    ...
    /// The next payment date is advanced deterministically from the previous due date
    /// to prevent schedule drift, regardless of when the payment is actually made.
    ///
    /// # Date Progression Logic
    ///
    /// - If the current time is before or at the due date (early/on-time payment),
    ///   the next due date advances by exactly one interval from the previous due date.
    /// - If the current time is past the due date (late payment), the next due date
    ///   is calculated by advancing from the previous due date by one interval, then
    ///   checking if that new date is still in the past. If so, it continues advancing
    ///   until the next due date is in the future, preventing duplicate period coverage.
    ///
    /// # Arguments
    ///
    /// * `caller`    - Address of the policyholder (must sign).
    ...
    pub fn pay_premium(env: Env, caller: Address, policy_id: u32, amount: i128) -> bool {
        ...
        // Deterministic date progression: advance from previous due date, not current time.
        // This prevents drift when payments are early or late.
        let mut next_due = policy.next_payment_due.saturating_add(PREMIUM_INTERVAL_SECONDS);

        // If the new due date is still in the past, advance until it's in the future.
        // This handles late payments covering only one period at a time.
        while next_due <= now {
            next_due = next_due.saturating_add(PREMIUM_INTERVAL_SECONDS);
        }

        policy.next_payment_due = next_due;
