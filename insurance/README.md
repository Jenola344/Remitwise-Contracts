...
5. **Deterministic payment schedule** — the `next_payment_due` date advances from
   the previous due date, not the current timestamp. This prevents schedule drift
   that could result from early or late payments, ensuring predictable billing cycles
   and preventing exploitation through payment timing manipulation.
